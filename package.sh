...
if [ -f /etc/fedora-release ]; then
    ...
    sudo dnf install -y rpm-build ninja-build cmake gcc-c++ eigen3-devel mesa-libGL-devel gmsh-devel glfw-devel spectra-devel
    cmake -S . -B "$BUILD_DIR" -G Ninja -DCMAKE_BUILD_TYPE=Release
    cmake --build "$BUILD_DIR"
    cpack --config "$BUILD_DIR/CPackConfig.cmake" -G RPM
    echo "RPM created successfully in build/"
    ...
    sudo apt-get update
    sudo apt-get install -y cmake ninja-build build-essential pkg-config libeigen3-dev libglfw3-dev libgmsh-dev libspectra-dev libgl1-mesa-dev
    cmake -S . -B "$BUILD_DIR" -G Ninja -DCMAKE_BUILD_TYPE=Release
    cmake --build "$BUILD_DIR"
    cpack --config "$BUILD_DIR/CPackConfig.cmake" -G DEB
    echo "DEB created successfully in build/"
else
